...
namespace Aheadworks\Ca\Model\ThirdPartyModule\Aheadworks\RewardPoints\Model\Company;
...
class AssignmentProcessor implements AssignmentProcessorInterface
{
    ...
    {
        if (!$this->thirdPartyModuleManager->isAwRewardPointsModuleEnabled()) {
            return false;
        }

        /** @var \Aheadworks\RewardPoints\Api\PointsSummaryRepositoryInterface $summaryRepository */
        $summaryRepository = $this->objectManager->get(
            \Aheadworks\RewardPoints\Api\PointsSummaryRepositoryInterface::class
        );

        try {
            $newAdminSummary = $summaryRepository->get($newCompanyAdmin->getId());
            if ($newAdminSummary) {
                throw new LocalizedException(__('Provided user has its own reward points history'));
            }
        ...
        try {
            ...
            $transactionSource = $this->objectManager->get(
                \Aheadworks\RewardPoints\Model\ResourceModel\Transaction::class
            );
