...
class Updater
{
     ...
     */
    public function updateCreditLimit($companyId, $creditLimitData)
    {
        if (!$this->isAllowedToUpdate($companyId, $creditLimitData)) {
            return false;
        }

        /** @var \Aheadworks\CreditLimit\Model\Customer\Backend\BalanceUpdater $balanceUpdater */
        ...
        $balanceUpdater->updateIsAllowedToExceedFlag($rootUser->getId(), $creditLimitData);
     ...
     * @return bool
     */
    private function isAllowedToUpdate($companyId, $creditLimitData)
    {
        if (!$companyId || empty($creditLimitData)) {
            return false;
        }
