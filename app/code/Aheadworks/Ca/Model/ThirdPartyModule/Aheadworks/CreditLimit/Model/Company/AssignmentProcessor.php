...
class AssignmentProcessor implements AssignmentProcessorInterface
{
    ...
    public function process(CustomerInterface $oldCompanyAdmin, CustomerInterface $newCompanyAdmin): bool
    {
        ...
        /** @var \Aheadworks\CreditLimit\Api\SummaryRepositoryInterface $summaryRepository */
        $summaryRepository = $this->objectManager->get(
            \Aheadworks\CreditLimit\Api\SummaryRepositoryInterface::class
        );

        try {
            $newAdminSummary = $summaryRepository->getByCustomerId($newCompanyAdmin->getId());
            if ($newAdminSummary) {
                throw new LocalizedException(__('Provided user has its own credit limit history'));
            }
        ...
        try {
            $oldAdminSummary = $summaryRepository->getByCustomerId($oldCompanyAdmin->getId());
