...
class AwCaRoleModel implements ProcessorInterface
{
    /**
     * @var UserResolver
     */
    ...
    public function addCustomData(HistoryLogInterface $object, array $processor, AbstractModel $model): HistoryLogInterface
    {
        ...
        if ($model->isObjectNew()) {
             $object->setValuesSetTo(__('New Role %1', $model->getName())->render());
        } else {
             $object->setValuesSetTo(__('Updated Role %1', $model->getName())->render());
        }
        return $object;
    }
}
