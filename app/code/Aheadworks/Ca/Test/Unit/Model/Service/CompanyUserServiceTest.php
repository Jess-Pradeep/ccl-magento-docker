...
class CompanyUserServiceTest extends TestCase
{
    ...
    public function testGetCurrentUser()
    {
        ...
        $customerExtensionMock = $this->getMockBuilder(CustomerExtension::class)
            ->disableOriginalConstructor()
            ->onlyMethods(['getAwCaCompanyUser'])
            ->getMock();
    ...
    public function testGetRootUserForCustomer()
    {
        ...
        $this->searchCriteriaBuilderMock->expects($this->exactly(2))
            ->method('addFilter')
            ->willReturnOnConsecutiveCalls(['aw_ca_customer_by_company_id', $companyId], ['aw_ca_customer_is_root', null])
            ->willReturnSelf();
    ...
    public function testSaveUserOnNewUser()
    {
        ...
        $this->notifierMock->expects($this->once())
            ->method('notify')
            ->with($customerMock, Notifier::NEW_COMPANY_USER_CREATED);
