...
class QuoteUrl implements VariableProcessorInterface
{
    ...
    public function prepareVariables($variables)
    {
        ...
        if ($variables[EmailVariables::IS_SELLER]) {
            $quoteUrl = $this->url->getAdminQuoteUrl($quoteId);
        } else {
            $quoteUrl = $quote->getCustomerId()
                ? $this->url->getFrontendQuoteUrl($quoteId, $storeId)
                : $this->url->getExternalQuoteUrl($quote->getHash(), $storeId);
        }

        $variables[EmailVariables::QUOTE_URL] = $quoteUrl;
