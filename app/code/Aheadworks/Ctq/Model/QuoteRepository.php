...
class QuoteRepository implements QuoteRepositoryInterface
{
    ...
    {
        if (!isset($this->registry[$quoteId]) || $reload) {
    ...
    {
        if (!isset($this->registryByHash[$hash])) {
            $quoteId = $this->resource->getIdByHash($hash);
            if (!$quoteId) {
                throw NoSuchEntityException::singleField(QuoteInterface::HASH, $hash);
            }
            /** @var QuoteInterface $quote */
            $quote = $this->quoteInterfaceFactory->create();
            $this->resource->load($quote, $quoteId);
            $this->registry[$quoteId] = $quote;
            $this->registryByHash[$hash] = $quote;
        }

        return $this->registryByHash[$hash];
    }
