...
class QuoteManagement
{
    ...
    public function getCartByQuote($quote, $isNew = false, $storeId = null)
    {
        if ($this->cartCached === null) {
            ...
            if ($this->isOrderedQuote($quote)) {
                ...
                $cart = $this->getNativeCart($quote, $isNew);
                ...
                try {
                    ...
                    if (!$cart->getAwCtqSkipQuoteSaving()) {
    ...
    {
        $quote = $this->quoteRepository->get($quoteId);
        return $this->getNativeCart($quote);
    }
     ...
     * @throws LocalizedException
     */
    private function getNativeCart($quote, $isNew = false)
    {
        $cart = $this->convertToNativeCart->convert($quote->getCart(), $isNew);
        if ($quote->getCustomerId()) {
            //set in case cart was created from guest, but now it belongs to registered customer
