...
class Converter
{
    ...
    public function toObject($cartArray, $isNew = false)
    {
        ...
        $cartArray['items'] = $this->sortParentChildRelation($cartArray['items']);
        $cartItems = [];
    ...
    /**
     * Sorting items according to parent child relation
     *
     * @param array $items
     * @return array
     */
    private function sortParentChildRelation(array $items)
    {
        $children = [];
        $parents = [];
        foreach ($items as $item) {
            if (isset($item['parent_' . CartItemInterface::KEY_ITEM_ID])) {
                $children[] = $item;
            } else {
                $parents[] = $item;
            }
