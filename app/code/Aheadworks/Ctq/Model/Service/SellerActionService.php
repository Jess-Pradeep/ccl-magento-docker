...
use Aheadworks\Ctq\Model\Source\Quote\Action as QuoteAction;
...
class SellerActionService implements SellerActionManagementInterface
{
    ...
    public function getAvailableQuoteActions($quote)
    {
        ...
        $sellerActions = $statusRestrictions->getSellerAvailableActions();
        if (!$quote->getCustomerId() && in_array(QuoteAction::SELL, $sellerActions)) {
            unset($sellerActions[array_search(QuoteAction::SELL, $sellerActions)]);
        }

        return $this->actionManagement->getActionObjects($sellerActions);
    }
}
