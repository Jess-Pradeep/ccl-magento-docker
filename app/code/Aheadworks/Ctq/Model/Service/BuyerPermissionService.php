...
class BuyerPermissionService implements BuyerPermissionManagementInterface
{
    ...
    public function canRequestQuote($cartId)
    {
        try {
            ...
            $result = !$cart->getHasError()
                && $this->isAllowedForCurrentCustomerGroup($cart->getCustomerGroupId(), $cart->getStore())
                && $this->isAllowMinimumSubtotal($cart);
        } catch (\Exception $e) {
            $result = false;
        }
        return $result;
     ...
     * {@inheritdoc}
     */
    public function isAllowMinimumSubtotal($cart) {
        return $cart->getBaseSubtotal() >= $this->config->getMinimumQuoteSubtotal();
    }

    /**
     * {@inheritdoc}
     */
    public function canShowRequestQuoteButton($cartId)
    {
        try {
            $cart = $this->cartRepository->getActive($cartId);
            $result = !$cart->getHasError()
    ...
    public function canRequestQuoteList($cartId)
    {
        try {
            $cart = $this->cartRepository->getActive($cartId);
            $result = !$cart->getHasError()
                && $this->config->isQuoteListEnabled($cart->getStore()->getWebsiteId())
                && $this->isAllowedForCurrentCustomerGroup($cart->getCustomerGroupId(), $cart->getStore())
                && $this->isAllowMinimumSubtotal($cart);
        } catch (\Exception $e) {
            $result = false;
     ...
     * {@inheritdoc}
     */
    public function canShowRequestQuoteButtonQuoteList($cartId)
    {
        try {
            $cart = $this->cartRepository->getActive($cartId);
            $result = !$cart->getHasError()
    ...
    {
        return $this->config->isEnabledForCustomerGroupToRequestQuote($customerGroupId, $store->getWebsiteId());
    }
}
