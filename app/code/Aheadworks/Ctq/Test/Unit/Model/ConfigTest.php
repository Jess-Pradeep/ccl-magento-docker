...
class ConfigTest extends TestCase
{
    ...
    /**
     * Test isEnabledForCustomerGroupToRequestQuote method
     *
     ...
     * @param bool $result
     */
    public function testIsEnabledForCustomerGroupToRequestQuote($customerGroupId, $configValue, $result)
    {
        $websiteId = 9999;
        $this->scopeConfigMock->expects($this->once())
            ->method('getValue')
            ->with(
                Config::XML_PATH_GENERAL_CUSTOMER_GROUPS_TO_REQUEST_A_QUOTE,
                ScopeInterface::SCOPE_WEBSITE,
                $websiteId
            )->willReturn($configValue);

        $this->assertEquals($result, $this->model->isEnabledForCustomerGroupToRequestQuote($customerGroupId, $websiteId));
    }
     ...
     * @dataProvider boolDataProvider
     */
    public function testIsQuoteRequestAllowedFromCart($value)
    {
        $this->scopeConfigMock->expects($this->once())
            ->method('isSetFlag')
            ->with(Config::XML_PATH_GENERAL_IS_QUOTE_REQUEST_ALLOWED_FROM_CART, ScopeInterface::SCOPE_WEBSITE, null)
            ->willReturn($value);

        $this->assertEquals($value, $this->model->isQuoteRequestAllowedFromCart());
    }

    /**
     * Data provider for isEnabledForCustomerGroupToRequestQuote method
     */
    ...
    /**
     * Test isPdfAttachedToEmail method
     */
    public function testIsPdfAttachedToEmail()
    {
        $result = false;
        $this->scopeConfigMock->expects($this->once())
            ->method('isSetFlag')
            ->with(Config::XML_PATH_PDF_OPTIONS_IS_PDF_ATTACHED_TO_EMAIL)
            ->willReturn($result);

        $this->assertEquals($result, $this->model->isPdfAttachedToEmail());
    }
