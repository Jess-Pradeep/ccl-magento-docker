...
class Form extends AbstractEdit
{
    ...
    {
        if ($this->getIsGuestQuote() || $this->isQuoteDataSavedOnReload()) {
            return 'none';
        }
    ...
    {
        if ($this->isQuoteDataSavedOnReload()) {
            return 'none';
        }

        $storeId = $this->getStoreId();
        $customerId = $this->getCustomerId();
        if (($customerId !== null || $this->getIsGuestQuote()) && !$storeId) {
            return 'block';
        }
        return 'none';
    }
    ...
    public function getQuoteDataJson()
    {
        ...
        $data['save_data_on_reload'] = $this->isQuoteDataSavedOnReload();

        return $this->_jsonEncoder->encode($data);
    }

    /**
     * Check if quote data saved on reload
     *
     * @return bool
     */
    private function isQuoteDataSavedOnReload(): bool
    {
        return (bool)$this->_request->getParam('saveOnReload', false);
    }
}
