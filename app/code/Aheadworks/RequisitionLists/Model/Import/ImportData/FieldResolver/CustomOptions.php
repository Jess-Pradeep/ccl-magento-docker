...
class CustomOptions implements FieldResolverInterface
{
    /**
     * @var array|string[]
     */
    ...
    public function resolveData(string $data, string $field): array
    {
        ...
        try {
            if (in_array($field, $this->fieldsMap)) {
                foreach (explode(',', $data) ?? [] as $optionData) {
                    [$optionId, $optionValueId] = explode(':', $optionData);
                    if (isset($result[$optionId])) {
                        if (gettype($result[$optionId]) === 'string') {
                            $prevValue = $result[$optionId];
                            unset($result[$optionId]);
                            $result[$optionId][] = $prevValue;
                        }
